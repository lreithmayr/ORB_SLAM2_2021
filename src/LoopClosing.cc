...
namespace ORB_SLAM2
{
	...
	void LoopClosing::SetGridMapper(shared_ptr<GridMapping> GridMapper)
	{
		...
		while (nCandidates > 0 && !bMatch)
		{
			for (int i = 0; i < nInitialCandidates; i++)
			{
				...
				if (!Scm.empty())
				{
					...
					const int
						nInliers = Optimizer::OptimizeSim3(mpCurrentKF, pKF, vpMapPointMatches, gScm, 10, mbFixScale);
	...
	void LoopClosing::CorrectLoop()
	{
			 ...
			 vit != vend; vit++)
		{
			...
			for (
				vector<KeyFrame*>::iterator vit2 = mvpCurrentConnectedKFs.begin(), vend2 = mvpCurrentConnectedKFs.end();
				vit2 != vend2; vit2++)
			{
